...
<div class="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 overflow-hidden flex flex-col h-full relative">
    ...
    <button 
        ...
        class="absolute top-3 right-3 z-10 bg-white/80 backdrop-blur-sm p-2 rounded-full shadow-sm hover:bg-blue-600 hover:text-white transition-all text-gray-400 opacity-0 group-hover:opacity-100"
        title="So sánh"
    >
    ...
    <div class="absolute top-3 left-3 z-10 flex flex-col gap-2">
        ...
        @if($product->discount_percent > 0)
            <span class="bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">-{{ $product->discount_percent }}%</span>
        @endif
    </div>
    ...
    <div class="relative pt-[100%] overflow-hidden bg-gray-50 group-hover:bg-gray-100 transition-colors">
        ...
        <div class="absolute inset-x-0 bottom-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex justify-center z-10">
            @if(request('mode') == 'build')
                <a href="{{ route('build-pc', ['add' => 1, 'type' => $product->category->slug, 'id' => $product->id]) }}" 
                   class="w-full bg-blue-600 text-white font-bold py-3 rounded-xl shadow-lg hover:bg-blue-700 hover:scale-105 transition-all transform flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <form action="{{ route('cart.add', $product) }}" method="POST" class="w-full">
                    ...
                    <button type="submit" class="w-full bg-black text-white font-bold py-3 rounded-xl shadow-lg hover:bg-gray-800 hover:scale-105 transition-all transform flex items-center justify-center gap-2">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="p-5 flex flex-col flex-1">
        ...
        <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">{{ $product->category->name ?? 'Linh kiện' }}</span>
            @if($product->brand)
        ...
        {{-- Name --}}
        <h3 class="font-bold text-gray-900 text-base leading-snug mb-2 line-clamp-2 flex-1 group-hover:text-blue-600 transition-colors">
