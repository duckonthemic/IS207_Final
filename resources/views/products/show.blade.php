        ...
        <div class="container mx-auto px-4">
            ...
            <nav class="text-sm text-gray-500 mb-8 flex items-center gap-2">
                ...
                <a href="{{ route('products.index') }}"
                    class="hover:text-blue-600 transition-colors">{{ explode(' - ', $product->category->name)[0] ?? 'Sản phẩm' }}</a>
                <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="lg:col-span-7">
                    <div class="sticky top-24 space-y-4">
                        {{-- Main Image --}}
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 relative group overflow-hidden"
                            ...
                            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="showLightbox = true"
                                    class="bg-white text-gray-700 p-2 rounded-full shadow-md hover:bg-gray-50 transition-colors">
                            ...
                            <div x-show="showLightbox" @click="showLightbox = false"
                                ...
                                <div class="relative w-full max-w-5xl h-full flex items-center justify-center">
                                    ...
                                    <img :src="document.getElementById('mainImage').src" alt="{{ $product->name }}"
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 sticky top-24">
                        ...
                        <div class="mb-8">
                            @if($product->sale_price)
                                ...
                                <p class="text-sm text-green-600 font-medium">Tiết kiệm:
                                    {{ number_format($product->price - $product->sale_price, 0, ',', '.') }}₫</p>
                            @else
                                <span
                                    class="text-4xl font-bold text-gray-900">{{ number_format($product->price, 0, ',', '.') }}₫</span>
                            @endif
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 mb-8 space-y-3">
                            <div class="flex items-start gap-3">
                                <div
                                    class="w-5 h-5 rounded-full bg-green-100 text-green-600 flex items-center justify-center shrink-0 mt-0.5">
                            ...
                            <form action="{{ route('cart.add', $product) }}" method="POST" class="space-y-4">
                                ...
                                <div class="flex items-center gap-4">
                                    <div class="flex items-center border border-gray-200 rounded-xl bg-white">
                                        <button type="button" onclick="decrementQty()"
                                            class="px-4 py-3 text-gray-600 hover:bg-gray-50 rounded-l-xl transition-colors font-bold">-</button>
                                        <input type="number" name="quantity" id="quantity" value="1" min="1"
                                            max="{{ $product->stock }}"
                                            class="w-16 text-center border-x border-gray-200 py-3 focus:outline-none text-gray-900 font-medium">
                                        <button type="button" onclick="incrementQty()"
                                            class="px-4 py-3 text-gray-600 hover:bg-gray-50 rounded-r-xl transition-colors font-bold">+</button>
                                    </div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <button type="submit" name="action" value="add_to_cart"
                                        class="col-span-1 bg-black text-white font-bold py-4 rounded-xl hover:bg-gray-800 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                                        Thêm vào giỏ
                                    </button>
                ...
                <div class="lg:col-span-8 space-y-8">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                        <div class="flex items-center justify-between mb-8">
                            ...
                            <span
                                class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-sm font-medium">{{ $product->reviews_count }}
                                đánh giá</span>
                        </div>
                                    ...
                                    @for($rating = 5; $rating >= 1; $rating--)
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                                                <div class="h-full bg-yellow-400 rounded-full" style="width: {{ $percentage }}%">
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex gap-2" x-data="{ rating: 5, hover: 0 }">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <button type="button" @click="rating = {{ $i }}" @mouseenter="hover = {{ $i }}"
                                                        @mouseleave="hover = 0" class="text-2xl transition-colors focus:outline-none"
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="space-y-4">
                            <a href="#" class="block group">
                                <h4
                                    class="text-sm font-medium text-gray-800 group-hover:text-blue-600 transition-colors line-clamp-2 mb-1">
                    ...
                    <div class="rounded-2xl overflow-hidden relative group h-64">
                        ...
                        <div
                            ...
                            <p class="text-gray-300 text-sm mb-4">Tự xây dựng cấu hình máy tính mơ ước của bạn ngay hôm nay.
                            </p>
                            <a href="{{ route('build-pc') }}"
                                class="inline-block bg-white text-black font-bold py-2 px-4 rounded-lg text-sm hover:bg-gray-100 transition-colors text-center">
                                Bắt đầu ngay
                            </a>
                        </div>
                    </div>
