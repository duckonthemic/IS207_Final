...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-cyber-gray border border-cyber-border p-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="text-cyber-white font-mono">{{ number_format($order->items->sum(fn($item) => $item->price * $item->quantity), 0, ',', '.') }}₫</span>
                    </div>
                    @if ($order->total_discount > 0)
                    <div class="flex justify-between">
                        <span class="text-cyber-text-muted uppercase text-xs">Discount:</span>
                        <span class="text-green-400 font-mono">-{{ number_format($order->total_discount, 0, ',', '.') }}₫</span>
                    </div>
                    @endif
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between text-base font-black">
                        ...
                        <span class="text-cyber-white font-mono">{{ number_format($order->total_amount, 0, ',', '.') }}₫</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-cyber-border">
                    ...
                    <tr class="hover:bg-cyber-gray transition">
                        ...
                        <td class="px-6 py-4 text-right text-cyber-white font-mono">{{ number_format($item->price, 0, ',', '.') }}₫</td>
    ...
    <form action="{{ route('admin.orders.update', $order) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-cyber-black border border-cyber-border p-6">
            ...
            <select name="status" class="w-full bg-cyber-gray border border-cyber-border text-cyber-white rounded-none px-4 py-2 focus:border-cyber-white focus:outline-none transition mb-4 font-mono text-sm uppercase">
        ...
        <div class="bg-cyber-black border border-cyber-border p-6">
            ...
            <select name="payment_status" class="w-full bg-cyber-gray border border-cyber-border text-cyber-white rounded-none px-4 py-2 focus:border-cyber-white focus:outline-none transition mb-4 font-mono text-sm uppercase">
        ...
        <div class="md:col-span-2">
            <button type="submit" class="w-full px-6 py-4 bg-cyber-white text-cyber-black rounded-none hover:bg-gray-800 transition font-bold uppercase tracking-widest text-sm">
                Update Order Status
            </button>
        </div>
    </form>
    ...
    <div class="bg-cyber-black border border-cyber-border p-6 mb-6">
        ...
        <div class="bg-cyber-gray border border-cyber-border p-4 text-cyber-white space-y-2 text-sm font-mono">
            <p class="font-bold uppercase">{{ $order->orderAddress->full_name }}</p>
