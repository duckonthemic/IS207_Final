...
<div class="min-h-screen bg-cyber-black p-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="bg-cyber-black border border-cyber-border p-6 hover:border-cyber-white transition-colors group">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-cyber-text-muted text-xs font-bold uppercase tracking-wider mb-2">Tổng đơn hàng</p>
                    ...
                    <div class="w-12 h-12 bg-cyber-gray border border-cyber-border flex items-center justify-center group-hover:bg-cyber-white group-hover:text-cyber-black transition-colors text-cyber-white">
            ...
            <div class="bg-cyber-black border border-cyber-border p-6 hover:border-cyber-white transition-colors group">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-cyber-white font-mono">{{ number_format($totalRevenue, 0, ',', '.') }}₫</h3>
                        <p class="text-xs text-cyber-text-muted mt-2 font-mono">
                            <span class="text-green-400 font-bold">+{{ number_format($revenueToday, 0, ',', '.') }}₫</span> TODAY
                        </p>
                    </div>
                    <div class="w-12 h-12 bg-cyber-gray border border-cyber-border flex items-center justify-center group-hover:bg-cyber-white group-hover:text-cyber-black transition-colors text-cyber-white">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
            ...
            <div class="lg:col-span-2 bg-cyber-black border border-cyber-border p-6">
                <div class="flex items-center justify-between mb-6 border-b border-cyber-border pb-4">
                    ...
                    <a href="{{ route('admin.orders.index') }}" class="text-cyber-text-muted hover:text-cyber-white text-xs font-bold uppercase tracking-wider transition-colors">
                        Xem tất cả →
                    </a>
                </div>
                        ...
                        <table class="w-full text-left">
                            <thead>
                                <tr class="border-b border-cyber-border">
                                    <th class="text-cyber-text-muted text-xs font-bold uppercase tracking-wider py-3">Mã đơn</th>
                                    <th class="text-cyber-text-muted text-xs font-bold uppercase tracking-wider py-3">Khách hàng</th>
                                    <th class="text-right text-cyber-text-muted text-xs font-bold uppercase tracking-wider py-3">Tổng tiền</th>
                                    <th class="text-center text-cyber-text-muted text-xs font-bold uppercase tracking-wider py-3">Trạng thái</th>
                                    <th class="text-right text-cyber-text-muted text-xs font-bold uppercase tracking-wider py-3">Thời gian</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-cyber-border">
                                @foreach($recentOrders as $order)
                                    <tr class="hover:bg-cyber-gray transition-colors">
                                        <td class="py-3">
                                            <a href="{{ route('admin.orders.show', $order) }}" class="text-cyber-white hover:text-cyber-text-muted font-mono text-sm font-bold">
                                                {{ $order->code }}
                                            </a>
                                        </td>
                                        <td class="py-3 text-cyber-text text-sm">{{ $order->customer_name }}</td>
                                        ...
                                        <td class="py-3 text-center">
                                            @php
                                                $statusClasses = [
                                                    ...
                                                    'shipping' => 'text-indigo-400 border-indigo-400',
                                            ...
                                            @endphp
                                            <span class="inline-block px-2 py-1 border text-[10px] font-bold uppercase tracking-wider {{ $statusClasses[$order->status] ?? 'text-gray-400 border-gray-400' }}">
            ...
            <div class="bg-cyber-black border border-cyber-border p-6">
                <div class="flex items-center justify-between mb-6 border-b border-cyber-border pb-4">
                    ...
                    <span class="inline-flex items-center px-2 py-1 border border-red-500 text-red-500 text-xs font-bold font-mono">
                ...
                @if($lowStockProducts->count() > 0)
                    <div class="space-y-4">
                        @foreach($lowStockProducts as $product)
                            <div class="flex items-center gap-3 p-3 bg-cyber-gray border border-cyber-border hover:border-red-500 transition-colors group">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-bold text-cyber-white truncate group-hover:text-red-500 transition-colors">{{ $product->name }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-xs text-red-500 font-bold uppercase tracking-wider">Stock: {{ $product->stock }}</span>
                                        <span class="text-xs text-cyber-text-muted font-mono">SKU: {{ $product->sku }}</span>
                                    </div>
                                </div>
                                <a href="{{ route('admin.products.edit', $product) }}" class="flex-shrink-0 text-cyber-text-muted hover:text-cyber-white">
        ...
        <div class="bg-cyber-black border border-cyber-border p-6">
            <h2 class="text-lg font-black text-cyber-white mb-6 border-b border-cyber-border pb-4 uppercase tracking-wider">Doanh thu 7 ngày gần đây</h2>
