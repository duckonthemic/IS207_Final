...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <div class="flex items-center mb-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Chỉnh Sửa Thông Số</h1>
                <p class="text-gray-600 mt-1">{{ $specDefinition->name }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
        <form action="{{ route('admin.spec-definitions.update', $specDefinition) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="component_type_id" 
                    ...
                    <option value="{{ $type->id }}" {{ old('component_type_id', $specDefinition->component_type_id) == $type->id ? 'selected' : '' }}>
                        {{ $type->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <input type="text" name="name" value="{{ old('name', $specDefinition->name) }}" 
            ...
            <div class="mb-6">
                ...
                <input type="text" name="code" value="{{ old('code', $specDefinition->code) }}" 
            ...
            <div class="mb-6">
                ...
                <input type="text" name="unit" value="{{ old('unit', $specDefinition->unit) }}" 
            ...
            <div class="mb-6">
                ...
                <select name="input_type" 
                    ...
                    <option value="text" {{ old('input_type', $specDefinition->input_type) == 'text' ? 'selected' : '' }}>Text</option>
                    <option value="number" {{ old('input_type', $specDefinition->input_type) == 'number' ? 'selected' : '' }}>Number</option>
                    <option value="textarea" {{ old('input_type', $specDefinition->input_type) == 'textarea' ? 'selected' : '' }}>Textarea</option>
                    <option value="select" {{ old('input_type', $specDefinition->input_type) == 'select' ? 'selected' : '' }}>Select (dropdown)</option>
                </select>
                @error('input_type')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea name="options" rows="4" 
                          class="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:border-black focus:outline-none transition font-mono text-sm @error('options') border-red-500 @enderror">{{ old('options', is_array($specDefinition->options) ? json_encode($specDefinition->options) : $specDefinition->options) }}</textarea>
                @error('options')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $specDefinition->sort_order) }}" min="0"
            ...
            <div class="mb-6 space-y-3">
                <label class="flex items-center">
                    <input type="checkbox" name="is_required" value="1" {{ old('is_required', $specDefinition->is_required) ? 'checked' : '' }}
                           class="w-4 h-4 text-black border-gray-300 rounded focus:ring-black">
                    <span class="ml-2 text-gray-900 text-sm font-medium">Bắt buộc nhập</span>
                </label>

                <label class="flex items-center">
                    <input type="checkbox" name="is_filterable" value="1" {{ old('is_filterable', $specDefinition->is_filterable) ? 'checked' : '' }}
                           class="w-4 h-4 text-black border-gray-300 rounded focus:ring-black">
