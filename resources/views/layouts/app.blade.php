        ...
        <div class="container mx-auto flex items-center justify-between">
                    ...
                    <template x-for="product in compareList" :key="product.id">
                        <div class="relative group">
                            <img :src="product.image"
                                class="w-12 h-12 object-contain border border-gray-200 rounded bg-white p-1">
            ...
            <div class="flex gap-3">
                <button @click="compareList = []; localStorage.removeItem('compareList')"
                    class="text-gray-500 hover:text-red-500 text-sm font-medium">Xóa tất cả</button>
                <a :href="'/products/compare?ids=' + compareList.map(p => p.id).join(',')"
                    class="bg-gray-900 text-white px-6 py-2 rounded-lg font-bold hover:bg-gray-800 transition-colors shadow-lg">
                    So sánh ngay
                </a>
            </div>
        </div>
