...
@section('title', 'Thêm địa chỉ mới - UITech')
    ...
    <div class="bg-gray-50 min-h-screen py-8">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex items-center gap-2 text-sm mb-8 text-gray-500">
                ...
                <span class="text-gray-900 font-medium">Thêm địa chỉ mới</span>
            </nav>

            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-8 py-6 border-b border-gray-100">
                    <h1 class="text-xl font-bold text-gray-900">Thêm địa chỉ mới</h1>
                    <p class="text-sm text-gray-500 mt-1">Điền đầy đủ thông tin để thêm địa chỉ giao hàng</p>
                </div>
                    ...
                    <form action="{{ route('addresses.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex gap-4">
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="label" value="Home" {{ old('label', 'Home') == 'Home' ? 'checked' : '' }} class="hidden peer">
                                    <div
                                ...
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="label" value="Office" {{ old('label') == 'Office' ? 'checked' : '' }} class="hidden peer">
                                    <div
                                        class="p-4 border-2 border-gray-200 rounded-xl text-center peer-checked:border-gray-900 peer-checked:bg-gray-50 transition-all">
                                        <span class="text-2xl mb-2 block">🏢</span>
                                ...
                                <label class="flex-1 cursor-pointer">
                                    <input type="radio" name="label" value="Other" {{ old('label') == 'Other' ? 'checked' : '' }} class="hidden peer">
                                    <div
                                        class="p-4 border-2 border-gray-200 rounded-xl text-center peer-checked:border-gray-900 peer-checked:bg-gray-50 transition-all">
                                        <span class="text-2xl mb-2 block">📍</span>
                        ...
                        <div>
                            ...
                            <textarea name="address_line" id="address_line" rows="3" required
                                ...
                                placeholder="Số nhà, tên đường, tòa nhà, căn hộ...">{{ old('address_line') }}</textarea>
                            @error('address_line')
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                            @enderror
                        </div>
