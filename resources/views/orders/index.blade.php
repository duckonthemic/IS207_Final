    ...
    <div class="container mx-auto px-4 max-w-5xl">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
            <form method="GET" action="{{ route('orders.index') }}" class="flex flex-wrap gap-4 items-end">
                ...
                <div class="flex-1 min-w-[200px]">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" 
                               ...
                               class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-sm">
                    </div>
                </div>
                ...
                <div class="w-48">
                    ...
                    <select name="status" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all text-sm appearance-none cursor-pointer">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white hover:bg-blue-700 transition-all font-bold rounded-xl shadow-lg hover:shadow-blue-500/30 text-sm">
                        Lọc
                    </button>
                    @if(request('search') || request('status'))
                        <a href="{{ route('orders.index') }}" class="px-6 py-2.5 border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-all font-bold rounded-xl text-sm">
                            Xóa bộ lọc
                        </a>
                    @endif
                </div>
        ...
        @if($orders->isEmpty())
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                ...
                <a href="{{ route('products.index') }}" class="inline-block px-8 py-3 bg-blue-600 text-white hover:bg-blue-700 transition-all font-bold rounded-xl shadow-lg hover:shadow-blue-500/30">
                    Bắt đầu mua sắm
                </a>
            </div>
        @else
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
                        ...
                        <div class="p-6 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4 bg-gray-50/50">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-blue-600 font-bold text-xl">{{ number_format($order->total, 0, ',', '.') }}₫</p>
                            </div>
                        </div>
                        ...
                        <div class="p-6">
                                ...
                                @foreach($order->items->take(3) as $item)
                                    <div class="flex gap-4 items-center group">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-gray-900 font-medium truncate text-sm group-hover:text-blue-600 transition-colors">
                            ...
                            <div class="flex flex-wrap gap-3 border-t border-gray-100 pt-4">
                                <a href="{{ route('orders.show', $order) }}" class="flex-1 min-w-[150px] px-4 py-2.5 bg-gray-900 text-white hover:bg-gray-800 transition-all font-bold text-center rounded-xl text-sm shadow-lg hover:shadow-gray-500/30">
                                    Xem chi tiết
                                </a>
                                <form method="POST" action="{{ route('orders.reorder', $order) }}" class="flex-1 min-w-[150px]">
                                    @csrf
                                    <button type="submit" class="w-full px-4 py-2.5 border border-gray-200 text-gray-700 hover:bg-gray-50 hover:text-gray-900 transition-all font-bold rounded-xl text-sm">
                                        Đặt lại
                                    </button>
                                </form>
                                @if(in_array($order->status, ['pending', 'paid']))
                                    <form method="POST" action="{{ route('orders.cancel', $order) }}" 
                                        ...
                                        @csrf
                                        <button type="submit" class="w-full px-4 py-2.5 border border-red-100 text-red-600 hover:bg-red-50 hover:border-red-200 transition-all font-bold rounded-xl text-sm">
                                            Hủy đơn
                                        </button>
                                    </form>
                                @endif
