            ...
            <div class="flex items-center justify-center">
                <div class="flex items-center">
                    <div class="flex items-center text-gray-900">
                        <div class="h-10 w-10 bg-gray-900 text-white rounded-full flex items-center justify-center font-bold shadow-lg shadow-gray-900/20">1</div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
                    ...
                    @if($addresses->isEmpty())
                        <div class="text-center py-12 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
                            ...
                            <a href="{{ route('addresses.create') }}" class="inline-flex items-center justify-center px-6 py-3 bg-gray-900 text-white rounded-xl hover:bg-gray-800 transition-all font-bold shadow-lg hover:shadow-gray-900/20 transform hover:-translate-y-0.5">
                                Thêm địa chỉ mới
                            </a>
                        </div>
                    @else
                        <form method="POST" action="{{ route('checkout.store-shipping') }}" id="shipping-form">
                                ...
                                @foreach($addresses as $address)
                                    <label class="block cursor-pointer group relative">
                                        <input type="radio" name="address_id" value="{{ $address->id }}" 
                                               ...
                                               required>
                                        <div class="border-2 border-gray-100 rounded-xl p-6 transition-all peer-checked:border-gray-900 peer-checked:bg-gray-50 hover:border-gray-300">
                                            <div class="flex items-start justify-between">
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-3 mb-2">
                                                        <span class="font-bold text-gray-900 text-lg">{{ $address->fullname }}</span>
                                                        @if($address->is_default)
                                                            <span class="px-2.5 py-1 bg-gray-100 text-gray-700 text-xs font-bold rounded-full">Mặc định</span>
                                                        @endif
                                                    </div>
                                                ...
                                                <div class="ml-4">
                                                    <div class="w-6 h-6 rounded-full border-2 border-gray-300 peer-checked:border-gray-900 peer-checked:bg-gray-900 flex items-center justify-center transition-colors">
                                                        <div class="w-2.5 h-2.5 bg-white rounded-full opacity-0 peer-checked:opacity-100 transition-opacity"></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="absolute inset-0 border-2 border-gray-900 rounded-xl opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity"></div>
                                    </label>
                                @endforeach
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-4 pt-4 border-t border-gray-100">
                                ...
                                <button type="submit" class="flex-1 px-6 py-3.5 bg-gray-900 text-white rounded-xl hover:bg-gray-800 transition-all font-bold shadow-lg hover:shadow-gray-900/20 transform hover:-translate-y-0.5">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-24">
                            ...
                            <div class="flex gap-4 group">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex justify-between items-center">
                                        ...
                                        <p class="text-gray-900 text-sm font-bold">{{ number_format($item->subtotal, 0, ',', '.') }}₫</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="border-t border-gray-100 pt-4 mt-4">
                        <div class="flex justify-between items-end">
                            ...
                            <span class="text-2xl font-bold text-gray-900">{{ number_format($cart->getTotal(), 0, ',', '.') }}₫</span>
                        </div>
