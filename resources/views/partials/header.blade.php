        ...
        <div class="hidden md:flex items-center gap-6">
            ...
            <a href="{{ route('cart.index') }}" class="relative group">
                <div class="p-2 rounded-lg bg-cyber-card border border-cyber-border group-hover:border-cyber-accent group-hover:shadow-glow-cyan transition-all">
                    ...
                    @if(auth()->check() && auth()->user()->getActiveCart()?->getItemCount() > 0)
                        <span class="absolute -top-1 -right-1 w-5 h-5 bg-cyber-error text-cyber-darker text-xs font-bold rounded-full flex items-center justify-center">
                            {{ auth()->user()->getActiveCart()->getItemCount() }}
                        </span>
                    @endif
                </div>
            </a>
